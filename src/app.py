...
def add_favorite_character(people_id):
    ...
    user = User.query.get(user_id)
    if user is None:
        raise APIException("Cant find user", status_code= 404)
    
    character = Character.query.get(people_id)
    if character is None:
        raise APIException("Cant find character", status_code= 404)
    
    favorite = Favorite(name = character.name, user_id = user_id, character_id = people_id)
    db.session.add(favorite)
    db.session.commit()

    return jsonify({"message": "favorite character was added successfully"}), 200
...
def add_favorite_planet(planet_id):
    ...
    if planet is None:
        raise APIException("Cant find planet", status_code= 404)
    
    favorite = Favorite(name = planet.name, user_id = user_id, planet_id = planet_id)
    db.session.add(favorite)
    db.session.commit()

    return jsonify({"message": "favorite planet was added successfully"}), 200
...
def delete_favorite_planet(planet_id):
    ...
    return jsonify({"message": "favorite planet was deleted successfully"}), 200
...
def delete_favorite_character(people_id):
    ...
    return jsonify({"message": "favorite character has been deletes successfully"})
